...
def draw_skeleton(
    ...
    out_img = cv2.polylines(out_img, adjacent_keypoints, isClosed=False, color=(0, 255, 0))
    return out_img
...
def draw_skel_and_kp(
    ...
    for ii, score in enumerate(instance_scores):
        ...
        for ks, kc in zip(keypoint_scores[ii, :], keypoint_coords[ii, :, :]):
            keypointscorestest.append(ks)
            if ks < min_part_score:
                cv_keypointstest.append([0, 0])
                continue
            cv_keypoints.append(cv2.KeyPoint(kc[1], kc[0], 10. * ks))
            cv_keypointstest.append([kc[1], kc[0]])

    # print("Confidence Scores:\n")
    print("Keypoint Coordinates:\n", cv_keypointstest)
    # print(keypointscorestest)
